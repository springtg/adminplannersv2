...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : baseurl+'syslib/nicEdit/nicEditorIcons.gif',
			...
			if(type == 'title') {
				...
				switch(type) {
					case 'text':
						...
						break;
                                        case 'button':
						this.inputs[itm] = new bkElement('input').setAttributes({id : itm, 'value' : val, 'type' : 'button'}).setStyle({margin : '2px 0', fontSize : '13px', 'float' : 'left', height : '20px', border : '1px solid #ccc', overflow : 'hidden'}).setStyle(field.style).addEvent("click",fn).appendTo(contain);
						break;
                                        case 'img':
	...
	addPane : function() {
		...
		this.addForm({
			...
			'chooseimg' : {type : 'img', txt : '', style : {width: '16px',height:"16px",top:"32px",left:"220px",position:"absolute",border:"none",cursor:"pointer",background:"url(../../images/icon/16/pictures_folder_black.png) center no-repeat"},fn:function(){BrowseServer( 'Images:/', 'src' );}},
                        ...
                        'iheight' : {type : 'text', txt : 'Height', style : {width: '50px'}},
